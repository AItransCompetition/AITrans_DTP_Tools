...
parser.add_argument('--ip',type=str, help="the ip of dtp_server that required", required=True)
...
parser.add_argument('--numbers', type=int, default=60, help="the numbers that you can control ")
...
# os.system("sudo docker run --privileged -itd --name dtp_server y654125664/aitrans:1.0 /bin/bash")
#os.popen("sudo docker run --privileged -itd --name dtp_client y654125664/aitrans:1.0 /bin/bash")
#os.system("sudo docker restart dtp_server")
#os.system("sudo docker restart dtp_client")

client_run = "#!/bin/bash\ncd /root/AItrans\n" \
             "./client "+ server_ip + " " + port +" config/config-DTP.txt " + str(numbers) + " 1> client.log 2> client_logging"
...
print("completion rate of all blocks before deadline ：", (complete_sum / (sum - 1)) * 100, "%")
